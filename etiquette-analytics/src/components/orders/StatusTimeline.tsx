        ...
        return (
          <div key={status} className="flex items-center flex-1">
            <button
              ...
              disabled={readonly}
              className={`
                flex-1 min-h-[44px] py-2 px-3 rounded-lg text-sm font-medium transition-all
                ${readonly ? 'cursor-default' : 'cursor-pointer hover:ring-2 hover:ring-offset-1'}
                ${isCurrent ? ORDER_STATUS_COLORS[status] : 'bg-bg-secondary text-text-muted'}
                ${isCompleted && !isCurrent ? 'bg-bg-tertiary text-text-secondary' : ''}
              `}
            >
              <div className="flex flex-col items-center gap-1">
                <span className="text-xs">{ORDER_STATUS_LABELS[status]}</span>
                <div className="flex items-center gap-1">
                  <div className={`w-2 h-2 rounded-full ${isCompleted ? 'bg-current' : 'bg-border-primary'}`} />
                </div>
              </div>
            </button>
            {index < statuses.length - 1 && (
              <div className={`w-6 h-0.5 mx-1 ${isCompleted ? 'bg-text-muted' : 'bg-border-primary'}`} />
            )}
          </div>
        )
      })}
