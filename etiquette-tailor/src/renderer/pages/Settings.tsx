...
export default function SettingsPage() {
  ...
  const [restoringPieceTypes, setRestoringPieceTypes] = useState(false);
  ...
  const handleRestorePieceTypes = async () => {
    if (!window.confirm(t('This will delete all existing piece types and restore the defaults. Are you sure?'))) return;
    try {
      setRestoringPieceTypes(true);
      await window.electronAPI.pieceTypes.restoreDefaults();
      await loadData(); // Reload piece types
      window.alert(t('Default piece types have been restored successfully.'));
    } catch (err) {
      console.error('Failed to restore piece types:', err);
      window.alert(t('Failed to restore piece types. Please try again.'));
    } finally {
      setRestoringPieceTypes(false);
    }
  };
  ...
  const handleInvoiceSave = async () => {
    try {
      ...
      const notesArEl = document.querySelector('[name="invoice_notes_ar"]') as HTMLTextAreaElement | null;
      const notesEnEl = document.querySelector('[name="invoice_notes_en"]') as HTMLTextAreaElement | null;
      const termsArEl = document.querySelector('[name="invoice_terms_ar"]') as HTMLTextAreaElement | null;
      const termsEnEl = document.querySelector('[name="invoice_terms_en"]') as HTMLTextAreaElement | null;
      ...
      if (notesArEl) updates.invoice_notes_ar = notesArEl.value;
      if (notesEnEl) updates.invoice_notes_en = notesEnEl.value;
      if (termsArEl) updates.invoice_terms_ar = termsArEl.value;
      if (termsEnEl) updates.invoice_terms_en = termsEnEl.value;
      // Section order
    ...
    <div className="pb-12">
          ...
          <div className="bg-surface-container-lowest rounded-2xl p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <textarea
                  name="invoice_notes_ar"
                  rows={2}
                  className="input-field resize-none"
                  dir="rtl"
                  defaultValue={settings.invoice_notes_ar || ''}
                  placeholder={t('Shop responsibility disclaimer in Arabic')}
                />
              </div>
              <div>
                <label className="block text-xs font-semibold uppercase tracking-[0.05em] text-secondary mb-2 px-1">
                ...
                <textarea
                  name="invoice_notes_en"
                  rows={2}
                  className="input-field resize-none"
                  defaultValue={settings.invoice_notes_en || ''}
                  placeholder={t('Shop responsibility disclaimer in English')}
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-[0.05em] text-secondary mb-2 px-1">
                  {t('Terms & Conditions (Arabic)')}
                </label>
                <textarea
                  name="invoice_terms_ar"
                  rows={3}
                  className="input-field resize-none"
                  dir="rtl"
                  defaultValue={settings.invoice_terms_ar || ''}
                  placeholder={t('Payment and delivery terms in Arabic')}
                />
              </div>
              <div>
                <label className="block text-xs font-semibold uppercase tracking-[0.05em] text-secondary mb-2 px-1">
                  {t('Terms & Conditions (English)')}
                </label>
                <textarea
                  name="invoice_terms_en"
                  rows={3}
                  className="input-field resize-none"
                  defaultValue={settings.invoice_terms_en || ''}
                  placeholder={t('Payment and delivery terms in English')}
                />
              </div>
            </div>
          </div>
      ...
      {activeTab === 'preferences' && (
        <form onSubmit={handlePrefsSave} className="max-w-2xl space-y-6">
          ...
          <div className="bg-surface-container-lowest rounded-2xl p-8 space-y-6">
            <div className="flex items-center gap-3 mb-2">
              <span className="material-symbols-outlined text-error text-xl">restore</span>
              <h3 className="font-headline font-bold text-lg text-on-surface">{t('Data Management')}</h3>
            </div>
            <p className="text-xs text-secondary -mt-3">
              {t('Restore default data to the system. This will replace existing data.')}
            </p>

            <div className="flex items-center justify-between py-3 px-4 rounded-xl bg-error/5 border border-error/20">
              <div className="flex items-center gap-3">
                <span className="material-symbols-outlined text-error text-xl">styler</span>
                <div>
                  <div className="text-sm font-semibold text-on-surface">{t('Restore Default Piece Types')}</div>
                  <div className="text-xs text-secondary">
                    {t('Restore the 34 default piece types with their prices')}
                  </div>
                </div>
              </div>
              <button
                type="button"
                onClick={handleRestorePieceTypes}
                disabled={restoringPieceTypes}
                className="px-5 py-2.5 text-sm flex items-center gap-2 rounded-lg bg-error text-on-error hover:bg-error/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-semibold"
              >
                {restoringPieceTypes && (
                  <span className="material-symbols-outlined animate-spin text-base">progress_activity</span>
                )}
                {restoringPieceTypes ? t('Restoring...') : t('Restore')}
              </button>
            </div>
          </div>
        </form>
