...
export function initializeSchema() {
  ...
  // Sync log table for Supabase sync
  db.exec(`
  ...
  const invoiceKeys: [string, string][] = [
    ...
    ['invoice_header_text', 'Invoice / فاتورة'],
    ['invoice_shop_name_ar', 'إتيكيت للخياطة النسائية'],
    ['invoice_shop_name_en', 'Etiquette Tailor'],
    ['invoice_notes_ar', 'ملاحظة: المتجر غير مسؤول عن الأقمشة التي يجلها العميل'],
    ['invoice_notes_en', 'Note: We are not responsible for customer-provided fabrics'],
    ['invoice_terms_ar', '• الدفعة غير قابلة للاسترداد بعد استلام القطعة\n• التسليم خلال الموعد المحدد مشروط بتوفر الأقمشة'],
    ['invoice_terms_en', '• Payments are non-refundable after item collection\n• Delivery by promised date subject to fabric availability'],
  ...
  // Migration: Seed Supabase sync settings
  const supabaseSyncKeys: [string, string][] = [
    ['supabase_sync_enabled', '0'],
    ['supabase_sync_interval_minutes', '60'],
    ['supabase_last_sync', ''],
    ['supabase_sync_source', 'branch_' + (db.prepare('SELECT id FROM branches ORDER BY id LIMIT 1').get() as { id: number } | undefined)?.id || '1'],
  ];
  for (const [key, value] of supabaseSyncKeys) {
    insertIgnoreSetting.run(key, value);
  }
...
function migrateColumns() {
  ...
  // Add is_deleted to customers (soft delete support)
  if (!tables.customers?.includes('is_deleted')) {
    console.log('Migrating: adding is_deleted to customers');
    db.exec('ALTER TABLE customers ADD COLUMN is_deleted INTEGER DEFAULT 0');
  }

  // Add is_deleted to daily_production (soft delete support)
  if (!tables.daily_production?.includes('is_deleted')) {
    console.log('Migrating: adding is_deleted to daily_production');
    db.exec('ALTER TABLE daily_production ADD COLUMN is_deleted INTEGER DEFAULT 0');
  }

  // Add is_deleted to order_items (soft delete support)
  if (!tables.order_items?.includes('is_deleted')) {
    console.log('Migrating: adding is_deleted to order_items');
    db.exec('ALTER TABLE order_items ADD COLUMN is_deleted INTEGER DEFAULT 0');
  }
...
function seedSettings() {
  ...
  // Invoice text defaults
  insertSetting.run('invoice_header_text', 'Invoice / فاتورة');
  insertSetting.run('invoice_shop_name_ar', 'إتيكيت للخياطة النسائية');
  insertSetting.run('invoice_shop_name_en', 'Etiquette Tailor');

  // Invoice notes/disclaimer (bilingual)
  insertSetting.run('invoice_notes_ar', 'ملاحظة: المتجر غير مسؤول عن الأقمشة التي يجلها العميل');
  insertSetting.run('invoice_notes_en', 'Note: We are not responsible for customer-provided fabrics');

  // Invoice terms (bilingual)
  insertSetting.run('invoice_terms_ar', '• الدفعة غير قابلة للاسترداد بعد استلام القطعة\n• التسليم خلال الموعد المحدد مشروط بتوفر الأقمشة');
  insertSetting.run('invoice_terms_en', '• Payments are non-refundable after item collection\n• Delivery by promised date subject to fabric availability');
