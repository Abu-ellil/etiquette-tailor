...
export interface ElectronAPI {
 ...
 sync: {
    perform: () => Promise<{ success: boolean; pushed?: number; pulled?: number; errors?: string[]; lastSyncAt?: string }>;
    getStatus: () => Promise<{ enabled: boolean; intervalMinutes: number; lastSync: string | null; pendingChanges: number; syncSource: string }>;
    backfill: () => Promise<{ orders: number; customers: number; items: number; tasks: number; payments: number }>;
    enable: () => Promise<any>;
    disable: () => Promise<any>;
...
const api: ElectronAPI = {
  ...
  pieceTypes: {
    ...
    restoreDefaults: () => ipcRenderer.invoke('pieceTypes:restoreDefaults'),
  },
 ...
 sync: {
    perform: () => ipcRenderer.invoke('sync:perform'),
    getStatus: () => ipcRenderer.invoke('sync:getStatus'),
    backfill: () => ipcRenderer.invoke('sync:backfill'),
