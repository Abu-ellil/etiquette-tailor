...
export default function CuttingQueuePage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-4xl font-headline font-extrabold text-on-surface tracking-tight">{t('cuttingQueue.pageTitle')}</h1>
        <p className="text-secondary mt-1">{t('cuttingQueue.subtitle')}</p>
      </div>

      <div className="bg-surface-container-lowest rounded-xl p-6">
        <span className="text-xs font-bold tracking-widest uppercase text-secondary">{t('cuttingQueue.pendingCuts')}</span>
        <div className="flex items-baseline gap-2 mt-2">
          <span className="text-4xl font-extrabold text-primary">{tasks.filter((t) => t.status !== 'done').length}</span>
          <span className="text-secondary">{t('cuttingQueue.pieces')}</span>
        </div>
      </div>
      ...
      {tasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-secondary">
          ...
          <p className="font-headline font-bold text-lg">{t('cuttingQueue.noCuttingTasks')}</p>
          <p className="text-sm mt-1">{t('cuttingQueue.cuttingTasksWillAppear')}</p>
        </div>
      ) : (
              ...
              <div key={task.task_id} className={`bg-surface-container-lowest rounded-xl p-5 ${isDone ? 'opacity-50' : ''}`}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    {isOverdue && <span className="px-2 py-0.5 bg-error-container text-on-error-container text-xs font-bold rounded-full">{t('cuttingQueue.overdue')}</span>}
                  </div>
                  <div className="flex items-center gap-3">
                    {task.due_date && <span className="text-xs text-secondary">{t('cuttingQueue.due')}: {task.due_date}</span>}
                    {!isDone && (
                      <button
                    ...
                    {isDone && (
                      <span className="px-3 py-1 bg-tertiary-fixed text-on-tertiary-fixed rounded-full text-xs font-bold">{t('cuttingQueue.completed')}</span>
                    )}
                  </div>
                </div>
