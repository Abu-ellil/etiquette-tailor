...
export default function CuttingQueuePage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-4xl font-headline font-extrabold text-on-surface tracking-tight">Cutting Queue</h1>
        <p className="text-secondary mt-1">Today's cutting tasks sorted by delivery date</p>
      </div>

      <div className="bg-surface-container-lowest rounded-xl p-6">
        <span className="text-xs font-bold tracking-widest uppercase text-secondary">Pending Cuts</span>
        <div className="flex items-baseline gap-2 mt-2">
          <span className="text-4xl font-extrabold text-primary">{tasks.filter((t) => t.status !== 'done').length}</span>
          <span className="text-secondary">pieces</span>
        </div>
      </div>
      ...
      {tasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-secondary">
          ...
          <p className="font-headline font-bold text-lg">No cutting tasks</p>
          <p className="text-sm mt-1">Cutting tasks will appear here when assigned.</p>
        </div>
      ) : (
